...
class ZfScheduleParserTest {
    ...
    fun `解析出学生信息`() {
        ...
        assertEquals("2620926205", s!!.studentId)
        assertEquals("李萧宇", s.name)
    ...
    fun `课程名与教师正确`() {
        ...
        val db = courses.first { it.name == "数据库原理与应用" }
        assertEquals("卢凯", db.teacher)
    ...
    fun `星期几正确`() {
        ...
        assertEquals(
            2,
            courses.first { it.name == "数据库原理与应用" }.dayOfWeek
        )
        // 形势与政策 V - 星期四
        assertEquals(
            4,
            courses.first { it.name == "形势与政策 V" }.dayOfWeek
        )
        // 马克思主义基本原理 - 星期五
        assertEquals(
            5,
    ...
    fun `教室正确`() {
        val courses = ZfScheduleParser.parse(loadFixture()).courses
        assertEquals("B202", courses.first { it.name == "数据库原理与应用" }.location)
        assertEquals("C201", courses.first { it.name == "形势与政策 V" }.location)
    }

    @Test
    fun `一门课多条记录不被去重`() {
        // 数据库原理与应用 在 fixture 中出现 1 次；
        // Linux系统开发基础 出现 2 次（单周 + 双周，不同教师）
        val courses = ZfScheduleParser.parse(loadFixture()).courses
        assertEquals(
            "Linux 的单双周应各保留一条",
            2,
            courses.count { it.name == "Linux系统开发基础" },
        )
    }
    ...
    fun `双周课程在第1周不上课`() {
        val courses = ZfScheduleParser.parse(loadFixture()).courses
        // Linux系统开发基础（双周，2-18周）
        val even = courses.filter { it.name == "Linux系统开发基础" }
    ...
    fun `超出周次范围不上课`() {
        val courses = ZfScheduleParser.parse(loadFixture()).courses
        // 形势与政策 V：6-9周
        val c = courses.first { it.name == "形势与政策 V" }
