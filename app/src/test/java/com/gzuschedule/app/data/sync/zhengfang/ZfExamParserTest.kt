...
class ZfExamParserTest {
    ...
    fun `标准结构可解析`() {
        val raw = """
            {"items":[{"kcmc":"数据库原理","kssj":"2027-01-05 09:00~11:00",
                       "cdmc":"A101","zwh":"12","ksxs":"闭卷"}],"totalResult":1}
        """.trimIndent()
        val list = ZfExamParser.parse(raw, "2026-2027-1")
        assertEquals(1, list.size)
        val e = list[0]
        assertEquals("数据库原理", e.courseName)
    ...
    @Test
    fun `无课名的条目被跳过`() {
        val raw = """{"items":[{"cdmc":"A101"},{"kcmc":"有效课程"}]}"""
    ...
    @Test
    fun `校区与考场拼接`() {
        val raw = """{"items":[{"kcmc":"课","xqmc":"示例校区","cdmc":"A101"}]}"""
        assertEquals("示例校区 A101", ZfExamParser.parse(raw, "t").single().location)
    }

    @Test
    fun `只有考场无校区时也能显示`() {
        val raw = """{"items":[{"kcmc":"课","cdmc":"A101"}]}"""
        assertEquals("A101", ZfExamParser.parse(raw, "t").single().location)
    }
